...
WICHTIG: Verwende NIEMALS Backslashes (\) in deinem SQL!
- Schreibe COUNT(*) statt COUNT(\*)
- Schreibe beginn_datum statt beginn\_datum
- Backslashes sind in SQL nicht notwendig und führen zu Fehlern.
...
def nl_to_sql():
    ...
    # Erste Nachricht: Schema + erste Frage aus dem Verlauf (oder aktuelle Frage)
    messages = []

    if history:
        # Erstes Objekt: Schema + erste Frage
        first = history[0]
        messages.append({
            "role": "user",
        ...
        for entry in history[1:]:
            if entry.get("error"):
                messages.append({
                    "role": "user",
                    "content": f"Dieser SQL hat folgenden Fehler erzeugt:\n{entry['error']}\n\nBitte korrigiere das SQL."
                })
                messages.append({
                    "role": "assistant",
                    "content": entry["sql"]
        ...
        if last_history.get("error"):
            ...
            messages.append({
                "role": "user",
                "content": f"Dieser SQL hat folgenden Fehler erzeugt:\n{last_history['error']}\n\nBitte korrigiere das SQL. Antworte NUR mit SQL."
            })
        else:
